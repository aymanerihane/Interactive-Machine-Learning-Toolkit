...
class ChartHandler:
    def __init__(self, df,categorical_mappings=None):
        self._df = df
        self._categorical_mappings = categorical_mappings or {}
    ...
    def plot_scatter(self, x_column, y_column,target):
        ...
        self._categorical_mappings, it uses the original values for coloring.
        ...
        for column in [x_column, y_column]:
            if column in self._categorical_mappings:
                # Decode the categorical column using the mappings
                unique_values = self._categorical_mappings[target]
                color_labels = self._df[target].map({i: val for i, val in enumerate(unique_values)})
                legend_title = target
        ...
        if color_labels is not None:
            ...
            sns.scatterplot(x=self._df[x_column], y=self._df[y_column])
    ...
    def plot_box(self, categorical_column, numerical_column):
        ...
        Automatically decodes categorical column values using self._categorical_mappings if available.
        ...
        if categorical_column in self._categorical_mappings:
            unique_values = self._categorical_mappings[categorical_column]
            decoded_categories = self._df[categorical_column].map({i: val for i, val in enumerate(unique_values)})
            sns.boxplot(x=decoded_categories, y=self._df[numerical_column])
            plt.xlabel(f"{categorical_column} (decoded)")
        else:
            sns.boxplot(x=self._df[categorical_column], y=self._df[numerical_column])
    ...
    def plot_pie(self, column):
        ...
        unique_values_dict = self._categorical_mappings
        ...
        if unique_values_dict and column in unique_values_dict:
            ...
            legend_labels = self._df[column].unique()  # Fallback if no mapping is found

        # Plot the pie chart
        plt.figure(figsize=(10, 6))
        self._df[column].value_counts().plot.pie(
    ...
    def plot_violin(self, column):
        ...
        plt.figure(figsize=(10, 6))
        sns.violinplot(x=self._df[column])
    ...
    def plot_pair(self,sharedState):
        ...
        try:
            ...
            print(f"Error plotting pair plot: {e}")
            sns.pairplot(df)
        return plt.gcf()
    ...
    def plot_stacked_bar(self):
        ...
        plt.figure(figsize=(10, 6))
        self._df.groupby(self._df.columns[0]).sum().T.plot(kind='bar', stacked=True)
    ...
    def plot_line(self, x_column, y_column):
        ...
        plt.figure(figsize=(10, 6))
        sns.lineplot(x=self._df[x_column], y=self._df[y_column])
    ...
    def plot_bar_graph(self, column):
        ...
        plt.figure(figsize=(10, 6))
        sns.countplot(x=self._df[column])
    ...
    def plot_dendrogram(self):
        ...
        linked = linkage(self._df, 'ward')
        plt.figure(figsize=(10, 7))
        dendrogram(linked, orientation='top', labels=self._df.index, distance_sort='descending')
    ...
    def plot_cluster_scatter(self, x_column, y_column, labels):
        ...
        plt.figure(figsize=(10, 6))
        sns.scatterplot(x=self._df[x_column], y=self._df[y_column], hue=labels, palette='Set2')
